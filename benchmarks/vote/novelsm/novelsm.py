def launch_cmds_startup():
    print("Configuring for vote bench with novelsm")
...
def launch_cmds_server_gen(f, q, r, m, quorums, replicas, clients, ports):
    ...
    cmd=cmd + '/home/cyclone/cyclone/cyclone.git/benchmarks/vote/novelsm/novelsm_loader\n'
    f.write(cmd)
    cmd= ' echo ' + passwd + ' | sudo -S '
    cmd=cmd + 'cp -r /mnt/pmem0/votepreloaded /mnt/pmem0/voterocksdata\n'
    f.write(cmd)
    cmd=''
    ...
    cmd=cmd + ' LD_LIBRARY_PATH=/usr/lib:/usr/local/lib '
    cmd=cmd + '/home/cyclone/cyclone/cyclone.git/benchmarks/vote/novelsm/novelsm_server '
    cmd=cmd + str(r) + ' '
    cmd=cmd + str(m) + ' '
    cmd=cmd + str(clients) + ' '
...
def launch_cmds_client_gen(f, m, c, quorums, replicas, clients, machines, ports, bufsize):
    ...
    if m >= replicas:
        ...
        if c == 0 and m < replicas + client_machines:
            ...
            cmd=cmd + ' echo ' + passwd + ' | sudo -S '
            cmd=cmd + ' LD_LIBRARY_PATH=/usr/lib:/usr/local/lib '
            #cmd=cmd + '/home/cyclone/cyclone/cyclone.git/test/novelsm_client '
            cmd=cmd + '/home/cyclone/cyclone/cyclone.git/benchmarks/vote/novelsm/novelsm_async_client '
            cmd=cmd + str(c_start) + ' '
            cmd=cmd + str(c_stop) + ' '
            cmd=cmd + str(m) + ' '
...
def killall_cmds_gen(f):
    ...
    f.write('echo ' + passwd + ' | sudo -S pkill novelsm_client\n')
