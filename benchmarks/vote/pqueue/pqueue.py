...
def launch_cmds_server_gen(f, q, r, m, quorums, replicas, clients, ports):
    ...
    cmd=cmd + '/home/cyclone/cyclone/cyclone.git/benchmarks/vote/pqueue/pqueue_server '
    cmd=cmd + str(r) + ' '
    cmd=cmd + str(m) + ' '
    cmd=cmd + str(clients) + ' '
...
def launch_cmds_client_gen(f, m, c, quorums, replicas, clients, machines, ports,bufsize):
    ...
    if m >= replicas:
        ...
        if c == 0 and m < replicas + client_machines:
            ...
            cmd=cmd + '/home/cyclone/cyclone/cyclone.git/benchmarks/vote/pqueue/pqueue_async_client '
            cmd=cmd + str(c_start) + ' '
            cmd=cmd + str(c_stop) + ' '
            cmd=cmd + str(m) + ' '
