...
def launch_cmds_server_gen(f, q, r, m, quorums, replicas, clients, ports):
    ...
    cmd= cmd + 'rm -rf /mnt/pmem1/pmemcontainer\n'
    f.write(cmd)
    cmd=''
    cmd=cmd + 'echo ' + passwd + ' | sudo -S '
    cmd=cmd + ' PMEM_IS_PMEM_FORCE=1 '
    cmd=cmd + ' LD_LIBRARY_PATH=/usr/lib:/usr/local/lib '
    cmd=cmd + '/home/pfernando/cyclone/cyclone.git/test/blizmap_server '
    cmd=cmd + str(r) + ' '
    cmd=cmd + str(m) + ' '
    cmd=cmd + str(clients) + ' '
...
def launch_cmds_client_gen(f, m, c, quorums, replicas, clients, machines, ports,bufsize):
    if m >= replicas:
        ...
        if c == 0 and m < replicas + client_machines:
            ...
            cmd=cmd + '/home/pfernando/cyclone/cyclone.git/test/blizmap_async_client '
            cmd=cmd + str(c_start) + ' '
            cmd=cmd + str(c_stop) + ' '
            cmd=cmd + str(m) + ' '
...
def killall_cmds_gen(f):
    if os.environ.has_key('CYCLONE_PASS'):
        passwd=os.environ.get('CYCLONE_PASS')
    f.write('echo ' + passwd + ' | sudo -S pkill bliz_map_server\n')
    f.write('echo ' + passwd + '| sudo -S pkill bliz_map_client\n')
    f.write('echo ' + passwd + '| sudo -S pkill bliz_map_async\n')
