	...
	void accept(int available, int multicore)
	{
		...
		for(int i=0;i<available;i++) {
			...
			if(!multicore) {
				...
				adjust_head(m);
				rpc = pktadj2rpc(m);
				LT_START(trcekey_other,rpc); // message entry point into cyclone server
			}
			else {
...
*/
				...
				if(is_multicore_rpc(rpc)) { // Add a fresh head
					...
					/* fast path for update/write data packets */
					messages[accepted].data.buf = (void *)m;
					messages[accepted].data.len = pktadj2rpcsz(m);
					messages[accepted].type = RAFT_LOGTYPE_NORMAL;
					chain_tail = m;
...
#ifdef __COMMUTE
		   scheduler->schedule(app_callbacks.op_callback,app_callbacks.partition_callabck);
		   scheduler->gc();
#endif
